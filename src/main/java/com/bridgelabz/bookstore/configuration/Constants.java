package com.bridgelabz.bookstore.configuration;

public class Constants {
	
	private Constants() {}

	public static final String VERIFICATION_LINK = "http://localhost:8080/admin/verifyemail/";
	public static final String VERIFICATION_MSG = "Verification link";
	public static final String REST_LINK = "http://localhost:8080/resetpassword";
	public static final String RSET_PASSWORD = "Reset your password";

	public static final String SELLER_VERIFICATION_LINK = "http://localhost:8080/seller/verifyemail/";
	public static final String SELLER_VERIFICATION_MSG = "Verification link";
	
	public static final String USER_VERIFICATION_LINK =  "http://localhost:8080/user/verifyemail/";
	public static final String USER_VERIFICATION_MSG = "Verification link";
	
	public static final String SELLER_RESETPASSWORD_LINK="http://localhost:8080/resetpassword/";
	public static final String SELLER_RESET_PASSWORD = "Reset your password";
	
	public static final String USER_RESETPASSWORD_LINK="http://localhost:8080/resetpassword/";
	public static final String RESET_PASSWORD = "Reset your password";
}
